...
class OptunaFunc():
    ...
    """
    def __init__(
        self, wrapper, callbackDir, n_trials:int = 50, n_timesteps = 1000, eval_freq =100, 
    ...
    def sample_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        gamma = trial.suggest_float("gamma", 0.97, 0.995, log=True)
        tau = trial.suggest_float("tau", 0.0005, 0.005, log=True)
        learning_rate = trial.suggest_float("lr", 1e-4, 3e-3, log=True)
        ent_coef = trial.suggest_float("ent_coef", 0.0001, 0.3, log=True)
        
        batch_size = 2**trial.suggest_int("batch_size_num", 8, 12, log=True)
        learning_starts = 1000*trial.suggest_int("learning_starts", 5, 10, log=True) 
        buffer_size = 50000 #5000*trial.suggest_int("buffer_size_num", 1, 10, log=True) 
        train_freq = 50 #50*trial.suggest_int("train_freq_int", 1, 3, log=True)
        gradient_steps = train_freq # 100 #50*trial.suggest_int("gradient_steps_int", 1, 4, log=True)

        action_noise = 0.05*trial.suggest_int("action_noise_int", 1, 4, log=True) - 0.05 #minium is zero noise
        self.action_noise = action_noise


        net_arch_width_int = trial.suggest_int("net_arch_width_int", 5, 7)
        net_arch_width = 2 ** net_arch_width_int
        net_arch_depth = trial.suggest_int("net_arch_depth", 4, 6)
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        n_envs = trial.suggest_int("n_envs", 2, 5, log=True) 
