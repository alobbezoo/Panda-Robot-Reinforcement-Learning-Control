...
class OptunaFunc():
    ...
    def sample_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        batch_size = 512 #2**trial.suggest_int("batch_size_num", 7, 10, log=True)
        learning_starts = 1000*trial.suggest_int("learning_starts", 1, 10, log=True) 
        buffer_size = 5000*trial.suggest_int("buffer_size_num", 1, 10, log=True) 
        train_freq = 100 #50*trial.suggest_int("train_freq_int", 1, 3, log=True)
        gradient_steps = 100 #50*trial.suggest_int("gradient_steps_int", 1, 5, log=True)

        action_noise = 0.05*trial.suggest_int("action_noise_int", 1, 5, log=True) - 0.05 #minium is zero noise
        self.action_noise = action_noise


        # net_arch_width_int = trial.suggest_int("net_arch_width_int", 5, 8)
        net_arch_width = 256 #2 ** net_arch_width_int
        net_arch_depth = 3 #trial.suggest_int("net_arch_depth", 2, 4)
