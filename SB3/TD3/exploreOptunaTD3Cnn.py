...
class OptunaFunc():
    ...
    def sample_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        tau = trial.suggest_float("tau", 0.001, 0.04, log=True)
        learning_rate = trial.suggest_float("lr", 1e-4, 4e-3, log=True)
        
        batch_size = 2**trial.suggest_int("batch_size_num", 7, 11, log=True)
